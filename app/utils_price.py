...
def is_emoji_only_line(line: str) -> bool:
    ...
    Простая реализация без Unicode категорий.
    """
    if not line.strip():  # Пустая строка или только пробелы
        return False
    
    # Удаляем ВСЕ символы, которые МОГУТ быть в нормальном тексте
    # Оставляем только эмодзи и специальные символы
    cleaned = re.sub(
    ...
    # Если после очистки осталась непустая строка - значит это были только эмодзи/декор
    return len(cleaned) > 0

def clean_text_advanced(text: str) -> str:
    """
    Основная функция очистки текста:
    1. Удаляет строки с 'актив'/'предактив'/'active' И строку НАД ними
    ...
    while i < len(lines):
        ...
        # Проверяем, содержит ли текущая строка ключевые слова активности
        if contains_active_keyword(line):
            # Если предыдущая строка была товаром (и мы её ещё не удалили),
            # удаляем её тоже
            if cleaned_lines and is_price_line(cleaned_lines[-1]):
                cleaned_lines.pop()
            # Саму строку с "Актив" не добавляем
            i += 1
            continue
...
def replace_prices_in_text(
    ...
    - Заменяет 📱📱📱 на @perviykremlevskiy 📱
    - Удаляет строки с 'актив', 'предактив', 'active' И строку над ними
